...
-v /myh/mydata/nacos/logs:/home/nacos/logs \
-v /myh/mydata/nacos/data:/home/nacos/data \
-v /myh/mydata/nacos/conf/application.properties:/home/nacos/conf/application.properties \
--link mysql \
-d nacos/nacos-server:v2.0.4

https://blog.csdn.net/qq_37451395/article/details/125274071 【Docker】容器连接到mysql（容器互联 超详细）
```
...
docker run -p 3306:3306 --name mysql \
-v /myh/mydata/mysql/log:/var/log/mysql \
-v /myh/mydata/mysql/data:/var/lib/mysql \
-v /myh/mydata/mysql/conf:/etc/mysql \
-v /myh/mydata/mysql/mysql-files:/var/lib/mysql-files \
...
除了可以基于指定的镜像构建容器，还可以基于`Dockerfile`文件构建，在使用`up`命令时会执行构建任
...
如果服务器的时区不正确，可以在`docker-compose.yml`设置`environment`新增时区`TZ: Asia/Shanghai`，示例如下。
